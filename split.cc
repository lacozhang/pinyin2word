...
bool spliter::operator()( const std::vector<unsigned int>& src, trie& tree, MyMap& p2w, ptable& out ){
  int start, end;
  ...
  for( int i=0; i<src.size(); ++i ){
	phrase tmp;
	tmp.start = tmp.end = i;
	int next = (p2w.m_element[ src[i] ]).next;
	tmp.m_str.push_back( (p2w.m_element[ src[i] ]).value);
	out.insert( tmp );
	tmp.m_str.clear();
	while( next != -1 ){
	  tmp.m_str.push_back( (p2w.m_element[next]).value );
	  next = (p2w.m_element[next]).next;
	  out.insert( tmp );
	  tmp.m_str.clear();
	}
